...
api_xls = base_api.get_xls('fbb_test.xlsx',sheet_name)
...
class TestReverse(unittest.TestCase):
    ...
    def checkResult(self):
        if self.message == '0.0':
            self.assertEqual(str(self.content['retcode']),'0')
        else:
            #断言状态码
            self.assertEqual([str(self.content['responseCommonDto']['resultCode']),
                              str(self.content['responseCommonDto']['message'])],
                             sorted(self.message.split(',')))
